...
from python_qt_binding.QtWidgets import (
    QWidget, QGroupBox, QProgressBar, QLCDNumber, QHBoxLayout)
...
HC_BOARD_CHANNEL_NAMES = ["NANOTEC 1", "NANOTEC 2",
                          "NANOTEC 3", "NANOTEC 4", "N/C", "N/C", "N/C", "LC BOARD"]
HC_BOARD_SOFT_FUSE_LEVELS = [5, 5, 5, 5, 0, 0, 0, 5]
HC_BOARD_SOFT_FUSE_TRIPPED = [False, False,
...
class Aleph2PowerController(Plugin):
    ...
    def __init__(self, context):
        ...
        for i in range(LC_BOARD_CHANNEL_COUNT):
            for v in "5v", "12v":
                self.lc_board_outputs_publishers[v].append(rospy.Publisher(
        ...
        for i in range(LC_BOARD_CHANNEL_COUNT):
            for v in "5v", "12v":
                self.lc_board_outputs_subscribers[v].append(rospy.Subscriber(
    ...
    def create_UI(self):
        ...
        for i in range(LC_BOARD_CHANNEL_COUNT):
            for v, container, layout in (
                ...
                subcontainer.setCheckable(True)
                subcontainer.ui = loadUi(
                    self.ui_file_lc_delegate, subcontainer)
        ...
        for i in range(JETSON_RAILS_COUNT):
            ...
            subcontainer.ui = loadUi(
                self.ui_file_jetson_delegate, subcontainer)
    ...
    def slot_refresh_hc_current(self):
        ...
        for i in range(8):
            ...
            self.hc_board_power_acc[i] += 1.0/3600.0 * \
                max(self.hc_board_current[i], 0.0) * U
    ...
    def setup_signals(self):
        ...
        self.signal_power_status_update.connect(
            self.slot_refresh_power_monitor)
